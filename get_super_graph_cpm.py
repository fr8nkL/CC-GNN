...
# TODO
def get_node_community_dict_cpm(cpm_output_path, n_nodes):
    '''
    return two dicts: node2community, community2node
    community values are normalized to range [0, n_communities)
    ...
    ######     Remark    ######
    ###### ###### ###### ######
    Many nodes do not belong to a community in CPM, 
    need to mannually assign them to a community
    '''
    node2community = {}
    community2node = {}
    num_communities = 0
    with open(cpm_output_path, "r") as f:
        for line in tqdm(f):
            line = line.strip()
            if len(line) > 0:
                nodes = [int(x) for x in line.split()]
                community2node[num_communities] = nodes
                for node in nodes:
                    if node in node2community:
                        node2community[node].append(num_communities)
                    else:
                        node2community[node] = [num_communities]
                num_communities += 1
    print('num of communities found by CPM = {} | num of nodes with community = {}'.format(num_communities, len(node2community.keys())))
    print('assigning lonely nodes to communities of themselves...')
    for node in tqdm(range(n_nodes)):
        if node not in node2community:
            node2community[node] = [num_communities]
            community2node[num_communities] = [node]
            num_communities += 1
    print('num of communities after = {}'.format(num_communities))
    return node2community, community2node, num_communities

# TODO
def calculate_sim_scores_jaccard(n_communities, node2community, community2node):
    print('using similarity measure: node-based, jaccard')
    # sim[(A,B)] := similarity between community A and B
    sim = {}
    # union_size: dictionary
    # key: pair (tuple) of communities, smaller first
    # val: set of it members
    union_size = {}
    ...
    # first calcualte the number of overlapping nodes
    for i in tqdm(range(n_communities)):
        for j in range(i + 1, n_communities):
            if i not in community_set:
                community_set[i] = set(community2node[i])
            if j not in community_set:
                community_set[j] = set(community2node[j])
            score = len(community_set[i].intersection(community_set[j]))
            # only store useful info.
            if score > 0:
                sim[(i,j)] = score
                sim[(j,i)] = score
    
    # divide by community size
...
# TODO
def calculate_sim_scores_edge_overlapping(graph, node2community, community2node):
    """
    edge based similariy
    overlapping community, this means node2community also returns a list instead of a single community
    """
    print('using similarity measure: edge-based, allow overlapping of communities')
    # sim[(A,B)] := similarity between community A and B
    sim = {}
    # union_size: dictionary
    # key: pair (tuple) of communities, smaller first
    ...
    for idx in tqdm(range(len(src))):
        ...
        if not(edge[0] in node2community and edge[1] in node2community):
            continue
        src_communities = node2community[edge[0]] # this is a list
        dst_communities = node2community[edge[1]] # this is a list
...
def main(args):
    ...
    # TODO: changed preprocessing path and function
    ...
    cpm_output_path = os.path.join(args.output_dir, args.dataset + '.txt')
    # TODO: for CPM, need to add args "n_nodes"
    node2community, community2node, n_communities = get_node_community_dict_cpm(cpm_output_path, n_nodes) 
    ...
    np.savez_compressed(os.path.join(args.output_dir, args.dataset), # TODO: added '_cpm'
    ...
    # TODO
    if args.sim_measure.lower() == 'jaccard' or args.sim_measure.lower() =='node':
        sim = calculate_sim_scores_jaccard(n_communities, node2community, community2node)
    elif args.sim_measure.lower() == 'edge':
        sim = calculate_sim_scores_edge_overlapping(graph, node2community, community2node) # edge based similarity
    ...
    try:
        ...
        # fig.show()
        fig.savefig('figure/' + args.dataset + '_sim_vals_cpm_' + args.sim_measure + '.png') # TODO: added _cpm to all save path
    except:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sim-measure", type=str, default="edge", # TODO
                        help="similarity measure of pairs of communities, can be jaccard or node or edge")
    parser.add_argument("--output-dir", type=str, default="CPM/output", # TODO
